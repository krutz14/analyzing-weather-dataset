...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
#    banks[(banks['Self_Employed']=='Yes')& (banks['Loan_Status']=='Y')]
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    banks.pivot_table(index=['Gender','Married','Self_Employed'],values='LoanAmount',aggfunc=np.mean)
    """
    return df.pivot_table(index=[date_col],values=[agg_col],aggfunc=agg)
...
def convert(df,celsius):
    ...
    return df.apply(lambda x: ((x * 9/5) + 32) if x.name=="Temp (C)" else x)
